...
- Language policy reaches the execution layer through a seam, never by the execution layer learning the language. `temporaryInitializer` is resolved once per activation before either lane runs; the execution layer never learns what `nil` is, and no executor learns it independently.

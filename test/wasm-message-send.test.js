...
test('WASM tail sends re-enter normal Smalltalk dispatch and may resolve another WASM Block', async () => {
  ...
  const neutralActivation = await runtime.invocations.invokeBlock(
    objectRef('demo', sender.block.id),
    [target],
  );
  const wasmActivation = await runtime.invocations.invokeBlock(
    objectRef('demo', wasmSender.block.id),
    [target],
  );
  ...
  assert.equal(wasmSender.moduleArtifact.metadata.sendSites[0].message.value, 'echo:');
  ...
  const semantic = await runtime.images.putCodeArtifact('demo', {
    ...
    content: {
      ...
      value: JSON.stringify({
        ...
        body: {
          op: 'if',
          condition: {op: 'equals', left: {op: 'literal', value: integerValue(1)}, right: {op: 'literal', value: integerValue(1)}},
          then: {op: 'send', languageId: 'symmetric-smalltalk', receiver: {op: 'argument', index: 0}, message: {kind: 'text', value: 'yourself'}, arguments: []},

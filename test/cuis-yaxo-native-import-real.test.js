...
test('M4 acceptance: complete durable restart vertical currently stops at the parser top accessor', {skip: !enabled, timeout: 900_000}, async () => {
  const manifest = JSON.parse(await yaxoSemanticExport());
  const directory = await mkdtemp(join(tmpdir(), 'yaxo-m4-'));
  try {
    // Temporary exact RED assertion, not an M4 success claim. Importing the canonical top accessor must move this
    ...
    await assert.rejects(runM4Acceptance(join(directory, 'application.sqlite'), manifest, {
      ...
      assert.equal(error.selector, 'top');
      assert.match(error.message, /^Symmetric Smalltalk message not understood: top sent to yaxo-m4\/~runtime\/transient\/object\/\d+\/[0-9a-f-]+$/);
      return true;
    });
  } finally {

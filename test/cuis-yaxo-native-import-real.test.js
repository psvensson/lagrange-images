...
test('M4 acceptance: complete durable restart vertical currently stops at Character isAscii', {skip: !enabled, timeout: 900_000}, async () => {
  const manifest = JSON.parse(await yaxoSemanticExport());
  const directory = await mkdtemp(join(tmpdir(), 'yaxo-m4-'));
  try {
    // Temporary exact RED assertion, not an M4 success claim. Repairing Character isAscii must move this
    ...
    await assert.rejects(runM4Acceptance(join(directory, 'application.sqlite'), manifest, {
      ...
      name: 'SmalltalkMessageNotUnderstoodError', selector: 'isAscii',
      message: 'Symmetric Smalltalk message not understood: isAscii sent to yaxo-m4/smalltalk/character/6e',
    });
  } finally {

...
test('M4 acceptance: complete durable restart vertical currently stops at Array asSet', {skip: !enabled, timeout: 900_000}, async () => {
  const manifest = JSON.parse(await yaxoSemanticExport());
  const directory = await mkdtemp(join(tmpdir(), 'yaxo-m4-'));
  try {
    // Temporary exact RED assertion, not an M4 success claim. Repairing Array asSet must move this
    ...
    }), {name: 'SmalltalkMessageNotUnderstoodError', selector: 'asSet'});
  } finally {

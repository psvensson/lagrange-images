...
test('M4 acceptance: complete durable restart vertical currently stops at Character digitValue', {skip: !enabled, timeout: 900_000}, async () => {
  const manifest = JSON.parse(await yaxoSemanticExport());
  const directory = await mkdtemp(join(tmpdir(), 'yaxo-m4-'));
  try {
    // Temporary exact RED assertion, not an M4 success claim. Repairing Character digitValue must move this
    ...
    }), {name: 'SmalltalkMessageNotUnderstoodError', selector: 'digitValue', message: 'Symmetric Smalltalk message not understood: digitValue sent to yaxo-m4/smalltalk/character/30'});
  } finally {

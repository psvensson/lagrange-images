...
import {
  ...
  installSymmetricSmalltalkStandardImage, installSmalltalkSymbolProtocol,
...
for (const lane of ['neutral', 'wasm']) {
  test(`exhaustive-recovery: every write publishing native ${lane} Text asSymbol recovers`, async () => {
    const imageId = 'as-symbol-recovery';
    const forks = await forkableRuntime(async runtime => {
      await runtime.images.createImage({id: imageId});
      const stop = new Error('stop before Text asSymbol publication');
      let firstId;
      const images = new Proxy(runtime.images, {get(target, key) {
        const value = target[key];
        if (key === 'putCodeArtifact') return (image, input, options) => {
          if (input.metadata?.smalltalk === 'method' && input.metadata.selector === 'asSymbol'
            && input.id.startsWith('smalltalk/class/Text/method/')) {
            firstId = input.id;
            throw stop;
          }
    ...
    const install = images => installSmalltalkSymbolProtocol({
      images, imageId, lane,
        ...
        for (let failAt = 1; failAt <= total; failAt++) {
          await forks.withFork(async runtime => {
            ...
            const {block} = await installSymmetricSmalltalkBlock({
              images: runtime.images, imageId, id: 'recovered-symbol-conversion',
              source: "[ ('note' asSymbol == #note) and: [('λ𝄞' asSymbol asString = 'λ𝄞') and: ['λ𝄞' asSymbol == 'λ𝄞' asSymbol]] ]",
            });

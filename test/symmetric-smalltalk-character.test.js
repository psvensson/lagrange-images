          ...
          await base.withFork(async (runtime) => {
            ...
            assert.deepEqual(
              await evaluate(
                ...
                "[ :characterClass | ((characterClass codePoint: 955) == $λ) and: [ ('λ😀' size = 2) and: [ ($λ == ('λ' at: 1)) and: [ ($λ codePoint = 955) and: [ ($λ asciiValue == nil) and: [ ($: asciiValue = 58) and: [ ($0 isDigit) and: [ ($λ isDigit not) and: [ ($ª isLetter) and: [ ($× isLetter not) and: [ ($Z digitValue = 35) and: [ ($z digitValue = -1) and: [$\u200b isSeparator] ] ] ] ] ] ] ] ] ] ] ] ]",

...
`Character>>isDigit` at the start of its classification loop (`ra9`). Public `Character class>>codePoint:` now delegates to the existing canonical scalar interner (`0q8`). The exact RED assertion is temporary; the recovery tail has

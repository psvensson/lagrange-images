...
| portable graph bundle -> target Image records (import) | Graph bundle model (`src/graph/bundle.js`) | decided (ADR 0074, later slice) | One owner for both translation directions so identity/closure rules cannot diverge. Import mints fresh target ObjectRefs via the ADR 0067 atomic `putObjects` batch (mint-then-write for cycles), resolves declared externals against the target environment, commits atomically. Not implemented in the export slice. |

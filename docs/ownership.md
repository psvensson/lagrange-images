...
| Portable graph bundle semantics (closure, bundle-local identity, external-ref rule, canonical form, content identity) | Graph bundle model | `src/graph/bundle.js` (ADR 0074, EXPORT-ONLY first slice implemented) | Owns the language-neutral transitive-graph materializer. Consumes `referencesOfRecord` (edge enumeration AND canonical edge order stay owned there) and record payloads; never duplicates graph storage or re-implements per-kind traversal. Traversal order is driven by `referencesOfRecord`; the generic projection rewrites exactly the refs the edge owner enumerates (executable guard). `contentIdentity` = SHA-256 over the canonical form via the crypto provider; metadata/`updatedAt`/`_version`/source provenance/frontier excluded from identity. Import and an authorized export lane are separate future owners; the atomic heterogeneous creation owner NOW EXISTS (`ImageService.createRecords` — §H correction: `putObjects`/ADR 0067 is generic-Object-only, so the importer will consume `createRecords`, not `putObjects`). |
...
| GraphImageService durable records -> portable graph bundle (export) | Graph bundle model (`src/graph/bundle.js`) | current (ADR 0074, EXPORT-ONLY first slice) | Owns the closure walk + local-id assignment + internal/external classification + canonical assembly + content identity. Reads through `GraphImageService` (`getRecord`) and `referencesOfRecord`; never duplicates graph storage semantics or invents a second traversal. The Project coordinator is NOT the exporter; a Project graph materializer is a representation owner that calls this owner. |
| portable graph bundle -> target Image records (import) | Graph bundle model (`src/graph/bundle.js`) | decided (ADR 0074, importer NOT yet implemented) | One owner for both translation directions so identity/closure rules cannot diverge. Import commits the heterogeneous record batch through the atomic heterogeneous GraphImageService creation operation (`ImageService.createRecords`, insert-only, one backend transaction) — NOT via `putObjects` (ADR 0067 is generic-Object-only; see ADR 0074 §H correction). External resolution and bundle localId -> target ObjectRef minting are the importer's own concern. Not implemented. |

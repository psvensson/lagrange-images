...
| Host crypto/random primitive seam | Crypto provider | `src/support/crypto-provider.js` (contract + Node reference) + `src/support/default-crypto.js` (active-default registry) | Owns the narrow SYNCHRONOUS host primitive contract (`secureRandomBytes`/`sha256`/`aes256gcmEncrypt`/`aes256gcmDecrypt`/`uuid`) and the Node reference implementation. Computes primitives ONLY — it is NOT modeled on the broad `node:crypto` API. All policy (observation cursor format, AES-GCM IV/tag layout, what is authenticated, key derivation, parsing, integrity failure) stays in the consuming semantic module (e.g. `image-observation-binding`), never here. The contract is synchronous because `typeFingerprint`/version-token/observation-cursor run deep inside synchronous executor paths; hosts whose only crypto is async are a separate finding, not a shim target. |

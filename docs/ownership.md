...
| Cuis semantic export manifest -> ordinary image objects | Image creation-batch binding executor (ADR 0067) | current: `src/callable/` | Owns materializing the manifest into Package/Class/Method objects/refs via the authorized atomic creation batch; the toolchain does not create image objects. `Cuis-Base` refs stay reserved identity strings, never materialized. |

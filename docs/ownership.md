...
| Portable graph bundle semantics (closure, bundle-local identity, external-ref rule, canonical form, content identity) | Graph bundle model | decided: `src/graph/bundle.js` (ADR 0074, not yet implemented) | Owns the language-neutral transitive-graph materializer. Consumes `referencesOfRecord` (edge enumeration stays owned there) and record payloads; never duplicates graph storage or re-implements per-kind traversal. `contentIdentity` = SHA-256 over the canonical form via the existing crypto provider; metadata/`updatedAt`/source provenance/frontier excluded from identity. Frontier stability and authority stay OUTSIDE the bundle. |
...
| GraphImageService durable records -> portable graph bundle (export) | Graph bundle model (`src/graph/bundle.js`) | decided (ADR 0074, not yet implemented) | Owns the closure walk + local-id assignment + internal/external classification + canonical assembly + content identity. Reads through `GraphImageService` (`getRecord`) and `referencesOfRecord`; never duplicates graph storage semantics or invents a second traversal. The Project coordinator is NOT the exporter; a Project graph materializer is a representation owner that calls this owner. |

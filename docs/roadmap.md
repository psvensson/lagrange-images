...
- [x] correspond complete Cuis semantic class identities to already-proven native classes through
  one closed, name-independent mapping seam, so the package's own `Integer>>jsonWriteOn:` extension
  installs through the existing kernel Integer's existing MethodDictionary and real native integer
  receivers dispatch into it;

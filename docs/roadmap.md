...
- [x] measure and repair the freshly exposed `UnicodeString streamContents:` name-resolution RED
...
- [x] construct Characters through public `Character class>>codePoint:` and the existing interner (`0q8`);
- [x] classify ASCII digits through native `Character>>isDigit` (`ra9`);
...
- [x] intern native Text through `asSymbol` using existing canonical Symbol identity (`tqr`);
  ...
  and initializes its text through reached instance-side `string:` (`7iu`); the callback next
  reaches omitted `XMLElement>>addContent:` (`agp`);

...
- [ ] portable Project release materialization (DECIDED: ADR 0075 — ONE multi-root bundle per release with roots = selected member keys; manifest v1 per-member contentIdentity = whole-bundle hash (coarse-but-truthful, sharing-topology-safe); fully-closed material (internalize-all unpinned, reachable pinned ref => refuse; empty externals proves source identity cannot enter releaseId); capture coordinator owns a lazy stable-current read session shared by BOTH capture paths; one `lagrange-project-release-material/v1` package per release; installation pre-effect check "bundle root keys === release member keys" makes post-import semantic-mismatch failure impossible. First slice PARTIALLY DONE: the stable-current read session owner exists (`createStableCurrentReadSession` in `src/project/release-capture.js`; the existing direct-record capture routes through it with behavior preserved, old eager bracket machinery removed). Remaining: `src/project/graph-release-materialization.js` consuming the session facade; installation coordinator deferred)

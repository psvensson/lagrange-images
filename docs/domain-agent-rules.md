...
- Anything reaching into a method dictionary should use the class builder's readers — `methodBlockRef()` for one selector, `methodBindings()` for the whole set — rather than assuming a layout; two are legal at once. Both are ONE implementation (`selectorBindings`), so "which selectors" and "which Block" cannot drift apart — and the installer's read-for-update (`readMethodBindings`) reads through that same current-binding reader, adding only write-path facts, so a write planner and a browse reader cannot disagree about what is bound (bead lagrange-images-jtz.2). Both MethodDictionary representations apply the same corruption rules there. The migration reader and `isLegacyMethodDictionary` are still separate implementations of the same rules (bead lagrange-images-jtz.6), so check which reader you are looking at before claiming the file has only one. A representation-neutral reader owes the *same* corruption semantics as dispatch — a missing Shape is a dangling edge, duplicate selectors are refused, a slot holding anything but an unpinned Block ref is malformed — or it becomes a laxer way to read the same records. These readers answer for ONE Behavior; what a send resolves to across a superclass chain stays owned by `smalltalk-lookup.js`.

### Authorized native browsing

- `src/language/smalltalk-browse.js` (ADR 0087) is the ONE public seam for describing a native class or method. It composes existing owners — `readBehavior`, `findSmalltalkKernel`, `methodBindings`, the instance Shape, `authority/object-resource.js` — and decodes nothing itself. Adding a field means asking whether some owner already answers it; if none does, the answer is `null`, not a reconstruction.
- Two independent `object/read` checks, and neither is inherited: the Class (or Metaclass) object for class browsing, and additionally the method's Block for method browsing. A class's own MethodDictionary rides the class's check (it is the Class's storage representation, ADR 0049 decision 3); the Blocks it BINDS do not, because a Block is independently addressable. Never widen class authority to reach a method, and never infer authority from Project membership, a class reference or graph reachability.
- Validate caller input, then require, then read — always in that order, so a denied caller cannot use the seam as an existence oracle. Refuse a description field that would leak a Behavior slot id, a MethodDictionary bucket/tally/seal, a backend `_version`, an instance-Shape id, a method's code artifact or compiled representation, Spur/Cuis object identity, or an importer-private mapping.
- `layout: null` (declares no instance layout) and `layout: {instanceVariables: []}` (declares an empty one) are different answers, exactly as `instanceShape` of `nil` differs from an empty Shape. Never collapse them.
- `selectors` is the class's OWN protocol. An inheritance walk here would let one grant speak for ancestors the caller may not read; a browser composes the walk from separately authorized reads.
- A native-imported class browses through this same seam with the same result shape. Cuis provenance is optional metadata on a native result and is absent today; do not reconstruct it, or a method's source, from the importer's transient output, a deterministic id, or a walk over a Block's code artifacts. Browsing only — edit/rename/recompile need their own consumer, owner decision and ADR.
...
- `super` is a reserved pseudo-variable and **not a Value** (ADR 0089). It exists only as the receiver marker of a message send; a bare `super`, `super` as an argument and `super` as an assigned value are refused rather than answering a proxy object. A proxy would be a second receiver, and a super send has none.
- **`self` is unchanged; only lookup moves.** Lookup starts at `superclass(the DEFINING Behavior of the running method)` — never the receiver's class, never the receiver's class's superclass, never a class named in source, never a Block's metadata. Those coincide when the receiver's class *is* the defining class, so a two-level proof on an instance of the immediate subclass proves almost nothing. The falsifier is three levels with the super send written in the MIDDLE class and sent to an instance of the bottom one.
- The defining Behavior comes from ADR 0050 decision 5b's trusted dispatch frame and is **never reconstructed** by asking which dictionary holds the running Block — decision 5 already rejected that answer as neither unique nor trustworthy. No frame means no defining Behavior, so a super send fails closed rather than guessing one from the receiver.
- `super <selector>` lowers to an ordinary send to `$superSend`, exactly as `^` lowers to `$nonLocalReturn`. `lagrange-code` gains no operation and the compiler recognizes no *selector*. The compiled artifact carries the selector text and the arguments and names no class: a durable artifact must not be able to state a lookup starting point.
- The super primitive is the one **variadic** one, because the message it forwards is; its arity entry is a minimum and the exception is an explicit set, so every other primitive keeps its exact-arity guard.
- **It delegates.** `readBehavior` answers the defining Behavior, its own `superclass` edge is the starting point, `lookupSelector` performs the walk, and the invocation owner activates the result. A second superclass walker or MethodDictionary reader in the super facility is two owners for one rule; a structural test forbids it. There is deliberately no branch for a root-defined method either — a kernel-nil superclass terminates the shared walk on its first comparison, which *is* the ordinary message-not-understood outcome.
- **The callee gets the ACTUALLY resolved defining Behavior**, not the one the send started above and not the caller's. Reusing the caller's frame turns a chained `super` into unbounded recursion and hands the callee the wrong instance-variable permissions. The frame is attached by the invocation owner's `prepareResolvedDispatch`, through the same normalization and envelope an ordinary dispatch uses, so the callee OWNS it.
- Class-side needs no rule of its own: the metaclass chain is derived from the class chain (ADR 0044 decision 4), so `readBehavior(frame.definingBehavior).superclass` walks it. Never implement `instance class superclass` or any instance-side approximation — the real consumer is class-side.
- `super` needs a method home, refused at compile time exactly where `^` is. Inside a nested Block it works while the creating execution is alive, because ADR 0050 decision 10 restores that method's frame; a closure that outlived its execution fails closed (decision 10a) and must never be lent the caller's frame or given a persisted defining Behavior.
- A cascade on `super` is refused: the cascade lowering evaluates its receiver once into a hidden temporary and `super` denotes nothing to put there. Widen it only with a real consumer and its own cascade semantics.
- `super` is never rewritten at the Cuis import boundary, and no importer, package or dialect special case exists. An adapter that "fixed" a super send by rewriting it to `self` would silently change which method a package calls.
...
- `true`, `false`, `nil`, `self` and `super` are reserved words, enforced in one place (`isReservedWord`) for all four sites: block parameters, temporaries, assignment targets and explicit captures. They had already drifted once — `self` was refused as a temporary but accepted as a block parameter.

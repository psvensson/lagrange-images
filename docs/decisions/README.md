...
- [0053 — Integer ordering and arithmetic](0053-integer-ordering-and-arithmetic.md): `<`, `<=`, `>`, `>=` and the remaining arithmetic are ordinary Smalltalk methods backed by language-owned Integer primitives, so `lagrange-code` stays frozen and the neutral IR never learns Smalltalk's numeric protocol.
...
- [0073 — Project release and installation boundary](0073-project-release-installation-boundary.md): keep Image, working Project, immutable release manifest and target-specific installation distinct. Project/member identities are semantic rather than target ObjectRefs; DeploymentProfile/v1 selects explicit member keys; release identity is canonical/content-addressed and separate from source-ref/frontier provenance; cross-image provenance uses ADR 0071's `{imageId -> revision}` map; upgrades are pure `install/retain/replace/detach` plans with detach explicitly not delete. Lossless graph bundles, materializers, durable deployment/recovery and Git projection/sync remain follow-up work.

...
| `installSmalltalkCharacterProtocol()` | the canonical image-local Character Shape/class and shared literal/Text-index/public `Character class>>codePoint:` interner (ADR 0090), plus ordinary `Character >> codePoint` over the existing scalar slot, partial `asciiValue` (ASCII scalars only, canonical nil outside 0–127), and the exact pinned `isSeparator` (seven scalars) and `isDigit` (ASCII 48–57) classifications. `isLetter` matches all pinned Latin-1 scalars 0–255 and explicitly refuses wider classification via `isLetterOutsideLatin1`; it composes ordinary Integer ranges and Boolean branches. Classification composes `codePoint`, Integer equality and lazy Boolean `or:`; it adds no primitive, host Unicode rule, generic Array membership or second state/recovery owner. |

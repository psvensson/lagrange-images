...
| `smalltalk-kernel-primitive/v1` | `installSmalltalkBlockProtocol()` | Publishes validated language-loop targets for classless Block `whileTrue:` / `whileFalse:` and unary `whileFalse`. Unary false-loop routing uses the same primitive with no body; conditions still run through ordinary `value` sends, require canonical Booleans and answer image-local nil. Binary body validation and frame/authority rules remain unchanged; no compiler loop instruction or new primitive is added (ADR 0051). |
| `installSmalltalkAllocationProtocol()`, `installSmalltalkIndexedProtocol()` | `createSmalltalkKernelPrimitiveV1Executor()`, registered by `createRuntime()` |
...
| `installSmalltalkEqualityProtocol()` | the `built-in-equals`/`built-in-hash` primitive Blocks, plus `Object >> =`, identity `Object >> ==`, its ordinary dynamically dispatched Boolean complement `Object >> ~~`, and `Object >> hash` (ADR 0048). `~~` is compiled Smalltalk source over `==` and `not`; there is no inequality primitive or compiler/importer rule. |
...
| `installSmalltalkArrayEnumerationProtocol()` | Native Array `do:` and `atAllPut:` after Block/Integer protocol installation. Fill delegates every existing index to ordinary `at:put:`, preserves receiver identity and length, stores the supplied Value/ref, and answers the receiver even when empty. No bulk-write primitive. |
| Native Text scalar sequence (`smalltalk-character.js`) | `size` uses the same scalar interpretation as `at:`; `do:` is ordinary Smalltalk over those methods and Integer/Block iteration. Each callback receives the canonical Character, once per Unicode scalar in order; callback answers are ignored, failures propagate, and the receiver is returned. Empty text makes no callback. The pure language-local `text-size` primitive counts no bytes/code units and performs no callback. |
| the `$array` intrinsic (`symmetric-smalltalk-semantic.js` / `-compiler.js`) | literal Arrays allocate through `new:` sent to this intrinsic; nonempty forms then use ordered ordinary `at:put:` sends and one private temporary per literal in `lagrange-code/v1`. The empty `#()` form retains its single v0 send. The compiler names no class and bakes no image-local ref (installation binds the intrinsic to `smalltalk/class/Array`), exactly as `$symbol`/`$nil`. Element positions accept existing literal kinds, bare identifier Symbols, Boolean/nil literals and nested `#(...)` Arrays; executable expressions remain refused. Forced by unchanged YAXO `XMLTokenizer class>>initialize` (bead x4i). |
| `installSmalltalkSymbolProtocol()` | the Symbol Shape/class, `symbol-intern`/`perform-send`/`perform-send-with` primitives, `Symbol >> asString`, `Object >> perform:`/`perform:with:`, and ordinary Text `asSymbol` calling the same captured interner as literals. Exact spelling (including empty/Unicode Text) and canonical image-local identity stay with the existing interner. Conversion adds no primitive, parser syntax, hierarchy or cache. |
| `installSmalltalkStringTestingProtocol()` | Allocation/class-introspection owner installs ordinary Object `isString` false and Text/Symbol true after Symbol protocol exists. Character and other classes inherit the default; application subclasses may override through ordinary method lookup. No host kind switch, new primitive or class hierarchy. Base allocation prerequisites remain unchanged. |
| `installSmalltalkCharacterProtocol()` | the canonical image-local Character Shape/class and shared literal/Text-index/public `Character class>>codePoint:` interner (ADR 0090), plus ordinary `Character >> codePoint` over the existing scalar slot, `isAscii` (exactly scalars 0–127), uppercase-only `digitValue` (0–9/A–Z map to 0–35, all other scalars to -1), partial `asciiValue` (ASCII scalars only, canonical nil outside 0–127), and the exact pinned `isSeparator` (seven scalars) and `isDigit` (ASCII 48–57) classifications. `isLetter` matches all pinned Latin-1 scalars 0–255 and explicitly refuses wider classification via `isLetterOutsideLatin1`; it composes ordinary Integer ranges and Boolean branches. Classification composes `codePoint`, Integer equality and lazy Boolean `or:`; it adds no primitive, host Unicode rule, generic Array membership or second state/recovery owner. |
| `installSmalltalkCharacterRangeProtocol()` | Separate post-global Character `to:` constructor: fresh native Array of inclusive canonical Characters using ordinary Array allocation/indexed writes, Integer iteration and public `Character class>>codePoint:`. Equal bounds make one element, adjacent descending bounds make none, and wider descending bounds fail existing negative-size validation. Installed-method and public-global prerequisites are checked before publication; basic Character interning retains its smaller dependency surface. Proven in both lanes and at every interrupted installation write. |
| `installSmalltalkTextByteArrayProtocol()` | the byte-sequence primitive family (`text-utf8-bytes`, private `unicode-scalar-utf8-bytes`, `bytearray-utf8-text`, `bytearray-size`, `bytearray-at`, `array-to-bytearray`) plus `Text >> utf8Bytes`/`asString`/`isEmpty`, `ByteArray >> utf8Text`/`size`/`at:`, and `ByteArray class >> fromArray:`. The scalar encoder is captured only by WriteStream after ordinary `Character>>codePoint`; it publishes no Character conversion selector and keeps all UTF-8 production in this existing codec owner. `Text>>isEmpty` is the one ordinary result observation forced by ADR 0091's unchanged tokenizer and is implemented through existing equality with empty Text, not a size primitive. A *separate* family from ADR 0047 indexed primitives: Text/bytes Values are immutable Values dispatching through kernel classes, not indexed image objects. UTF-8 is the only codec; ByteArray remains immutable — no `at:put:`. |
...
| `installSmalltalkIntegerPrintingProtocol()` | native `Integer >> printOn:base:`, ordinary Integer protocol usable by any native code, added because a real imported consumer sends it (bead lagrange-images-nv1.6: the pinned upstream Cuis JSON package's `Integer>>jsonWriteOn:` is `^ self printOn: aWriteStream base: 10`). It writes to its ARGUMENT through ordinary message sends, as the real protocol does, so the stream it is handed decides what write protocol is required — measured to be exactly one `nextPutAll:` per print. Digits come from the `//` and `\\` this file already installs and the text from the existing `Array` -> `ByteArray class >> fromArray:` -> `ByteArray >> utf8Text` conversion: no new primitive, and no character or text semantics invented inside Integer. Base 10 is the only base a consumer backs and the only base the proof claims. The method DECLARES the domain it is correct on — 2 to 36 — and refuses anything else by sending a selector nothing implements: without that, `base: 1` never terminates (`value // 1` is `value` forever, and the executor imposes no step budget) and a negative base silently answers punctuation, because `48 + digit` for a negative digit is still a legal byte. Within the declared range the digit-to-byte step needs the ordinary letter branch, and one base-16 case is checked against real Cuis so that branch is not unproven. Installed after the global namespace because its source names `Array` and `ByteArray`; ownership stays in `smalltalk-integer.js`. |
| Native ReadStream `next` (`smalltalk-read-stream.js`) | Ordinary cursor/read-limit comparison answers `collection at: (position := position + 1)` while input remains, and canonical nil at EOF without a write. Text `at:` supplies canonical scalar Characters; the stream adds no decoder or host object mutation. Replaying installation preserves consumed input. Public cursor accessors, stream `peek` and `atEnd` are not implied. |
| `installSmalltalkReadStreamProtocol()` / `installSmalltalkTextReadStreamProtocol()` | Ordinary native ReadStream construction: class/instance `on:` allocates and initializes collection, zero position and scalar read limit; Text `readStream` names the published ReadStream global. Class/layout/initialization belong to `smalltalk-read-stream.js`; scalar Text size stays with the Character/Text owner. Standard-image composition publishes the class between the stages. No host stream, new primitive, public cursor or read protocol is implied. Proven in both lanes and after every interrupted installation write. |
| `installSmalltalkSetProtocol()` / `installSmalltalkArraySetConversion()` | Ordinary native Set class with private Dictionary membership and `initialize`, `add:`, `includes:`, `size`, `do:`. Dictionary remains authoritative for equality/hash deduplication; `add:` answers the supplied argument. Standard-image composition publishes Set between the two stages, so Array `asSet` compiles through the normal global binding and enumerates its source into a fresh Set without changing the Array. No host Set representation or primitive. |
| `installSmalltalkWriteStreamProtocol()` | an ordinary native `WriteStream`, published as an ordinary global, holding two instance variables (`collection`, and one private ordered accumulation), plus the native convenience method `Text class >> streamContents:` (ADR 0091). WriteStream has six selectors: `class >> on:`, instance `on:`, `nextPut:`, `nextPutAll:`, `contents` and `reset`. Reset clears the one private accumulation and returns the same stream, so subsequent writes cannot retain a stale suffix. Both writes append tagged entries to the same state and answer the stream, while `contents` remains the only result constructor; the private tag preserves the element/chunk distinction without separate channels. Text chunks use `Text>>utf8Bytes`; Character elements are retained semantically until `contents`, which sends ordinary `codePoint` and delegates scalar encoding to the private primitive at the existing Text/ByteArray codec owner. WriteStream knows no Character object id, slot, record or UTF-8 arithmetic. `Text class >> streamContents:` captures the installed WriteStream Class, constructs it on empty Text, evaluates its producer Block once, ignores the Block answer, and returns `contents`; it owns no accumulation or reconstruction. The pinned oracle proves the upstream UnicodeString operation and its Utf8EncodedWriteStream `nextPut:` have the same evaluation/result, exact self return, ordered ASCII/BMP/supplementary and mixed element/chunk semantics on the forcing path; native results are honestly canonical Text, not simulated UnicodeString objects. Empty/absent writes contribute nothing. `with:`, arbitrary positioning, reading a WriteStream, mutable String, arbitrary collection-element writes and the Cuis PositionableStream/Utf8EncodedWriteStream hierarchy remain absent. The Cuis adapter translates only the exact foreign receiver locator to `Text streamContents:` and owns none of this execution. |
...
decision 6 as amended): **E3 exposes no lane choice, and for a replacement guarded by an observed
native revision the native method evolution owner preserves that revision's execution lane.** WASM
stays WASM and neutral stays neutral. It is a rule about native methods rather than about imported
ones — a native method replacement preserves its existing native execution policy, whatever produced
it — and imported Cuis material is simply the first consumer to notice, because every Cuis-imported
method is installed in the WASM lane.

Replacing what a native method MEANS is never also a migration of how it RUNS, and there is no
fallback: if the observed lane cannot compile the replacement source, the replacement fails and the
observed revision stays current. The token already names the observed immutable binding, so the
Environment never has to know what a lane is. An observation that is not a native method revision of
this position — absent, not a method, a method of another selector, or recording no known lane — is

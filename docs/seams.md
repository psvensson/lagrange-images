...
| `installSmalltalkWriteStreamProtocol()` | an ordinary native `WriteStream`, published as an ordinary global, holding two instance variables (`collection`, and a provisional internal accumulation). Four selectors and no more: the three the consumer sends, `WriteStream class >> on:`, `WriteStream >> nextPutAll:` and `WriteStream >> contents`, plus the instance-side `WriteStream >> on:` the class-side one delegates to — a split that is upstream's own (measured: instance-side `on:` in `WriteStream`, class-side in `PositionableStream class`) and is forced here anyway, because a metaclass method cannot assign an instance variable. That is the whole protocol the first real consumer exercises (bead lagrange-images-nv1.4: the pinned upstream Cuis JSON package writes `WriteStream on: String new`). Anchored to a recorded real-Cuis oracle: `on:` positions at the BEGINNING and DISCARDS the argument's existing content (append is `with:`, which is absent), and `contents` answers a fresh, species-preserving copy of the WRITTEN PREFIX. UNWRITTEN, `contents` is `collection species new`, which agrees exactly with upstream's `collection copyFrom: 1 to: position` for a stream nothing has written to. WRITTEN, it REFUSES, by signalling the named condition `WriteStreamContentsNeedsSpeciesPreservingResult`: answering an empty collection after data had been written would be a silent wrong answer reachable by any native user, and producing the real answer is the species question bead lagrange-images-nv1.7 owns. `nextPutAll:` answers THE STREAM (measured against the pinned image; the common Squeak/Pharo recollection that it answers its argument is wrong here) and accumulates into a PROVISIONAL, internal, unobservable representation, because upstream accumulates by mutating an indexed backing and the native path has no equivalent — the acceptance path's backing is an immutable Text Value, and this image has no text concatenation, no `replaceFrom:to:with:startingAt:` and no mutable String. `nextPut:` is still absent: no consumer sends it. Note one deliberate divergence: upstream implements `species` on `Object`, this image only on `Collection`, so `contents` works for a Collection backing and fails visibly (message-not-understood naming `species`) for an Array or a Value; adding `Object >> species` would close that and awaits a consumer that needs it. `nextPut:`, `with:`, positioning, resets and read streams are deliberately absent, and the Cuis `PositionableStream` hierarchy is not modelled. It also defines and publishes its own refusal condition class, `WriteStreamContentsNeedsSpeciesPreservingResult`. Nothing here is Cuis-specific: the import adapter resolves the name through the global namespace exactly as it resolves `Array`. |

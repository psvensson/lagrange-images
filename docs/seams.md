...
| `installSmalltalkCharacterProtocol()` | the canonical image-local Character Shape/class and shared literal/Text-index interner (ADR 0090), plus ordinary `Character >> codePoint` over the existing scalar slot and the exact pinned seven-scalar `Character >> isSeparator` classification. Classification composes `codePoint`, Integer equality and lazy Boolean `or:`; it adds no primitive, host Unicode rule, generic Array membership or second state/recovery owner. |

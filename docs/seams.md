...
| `installSmalltalkWriteStreamProtocol()` | an ordinary native `WriteStream`, published as an ordinary global, holding one instance variable (`collection`). Exactly two selectors — `WriteStream class >> on:` and `WriteStream >> contents` — because that is the whole protocol the first real consumer exercises (bead lagrange-images-nv1.4: the pinned upstream Cuis JSON package writes `WriteStream on: String new`). Anchored to a recorded real-Cuis oracle: `on:` positions at the BEGINNING and DISCARDS the argument's existing content (append is `with:`, which is absent), and `contents` answers a fresh, species-preserving copy of the WRITTEN PREFIX. With no write protocol that prefix is always empty, so `contents` is `collection species new` — exact today, and it MUST be revisited when the first write selector lands. `nextPut:`, `nextPutAll:`, `with:`, positioning, resets and read streams are deliberately absent, and the Cuis `PositionableStream` hierarchy is not modelled. Nothing here is Cuis-specific: the import adapter resolves the name through the global namespace exactly as it resolves `Array`. |

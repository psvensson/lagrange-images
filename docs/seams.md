...
| `installSmalltalkWriteStreamProtocol()` | an ordinary native `WriteStream`, published as an ordinary global, holding two instance variables (`collection`, and a provisional internal accumulation), plus the native convenience method `Text class >> streamContents:` (ADR 0091). WriteStream itself still has four selectors and no more: `class >> on:`, instance `on:`, `nextPutAll:` and `contents`. `Text class >> streamContents:` captures that installed Class, constructs it on empty Text, evaluates its producer Block once, ignores the Block answer, and returns `contents`; it owns no accumulation or reconstruction. This exact dependency is causal: replacing `WriteStream>>contents` changes the Text convenience method's answer. The pinned oracle proves the upstream UnicodeString operation has the same evaluation/contents composition, ordered BMP/supplementary Unicode behavior, and error propagation on the forcing path; native results are honestly canonical Text, not simulated UnicodeString objects. `contents` remains the only result owner: it builds from written chunks, preserving backing class, and text reconstruction uses the existing `utf8Bytes` -> `ByteArray class >> fromArray:` -> `ByteArray >> utf8Text` path. `nextPutAll:` answers the stream. Empty/absent writes contribute nothing. `nextPut:` remains absent: after product `Character>>isSeparator` moved the unchanged YAXO vertical through the exact seven-scalar classification, real execution of its separator branch reaches `WriteStream>>nextPut:` first; source inventory alone did not earn that later protocol before execution. `with:`, positioning, reset, read streams, mutable String and the Cuis PositionableStream/Utf8EncodedWriteStream hierarchy are also absent. The Cuis adapter translates only the exact foreign receiver locator to `Text streamContents:` and owns none of this execution. |
